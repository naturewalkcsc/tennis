...
function StartFromFixtures({ onBack, onStartScoring }) {
  ...
  const startFixture = async (fx) => {
    ...
    // SAFETY: call scoring handler only if provided (when running from admin app).
    if (typeof onStartScoring === "function") {
      ...
      // fallback (viewer/standalone): notify user and refresh fixtures list
      try {
        alert("Match marked Active: " + (fx.sides?.join(" vs ") || ""));
        const refreshed = await apiFixturesList();
