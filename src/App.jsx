...
import { Trophy, Play, Users, ChevronLeft, Plus, Trash2, CalendarPlus, RefreshCw, X, Eye } from "lucide-react";
...
/* -------- Local Admin Gate (no prefilled password) -------- */
...
/* -------- Viewer (public) -------- */
function Viewer(){
  const [fixtures,setFixtures]=useState([]); const [results,setResults]=useState([]); const [loading,setLoading]=useState(true);
  useEffect(()=>{ let alive=true;(async()=>{try{const fx=await apiFixturesList();const rs=await apiMatchesList();if(!alive)return;setFixtures(fx);setResults(rs)}finally{if(alive)setLoading(false)}})();const iv=setInterval(async()=>{try{setFixtures(await apiFixturesList());setResults(await apiMatchesList())}catch{}},10000);return()=>{alive=false;clearInterval(iv)}},[]);
  const active=fixtures.filter(f=>f.status==='active'); const upcoming=fixtures.filter(f=>!f.status||f.status==='upcoming'); const completed=fixtures.filter(f=>f.status==='completed');
  return (<div className="app-bg"><div className="max-w-5xl mx-auto p-6">
    <div className="flex items-center justify-between mb-6"><div className="flex items-center gap-3"><Eye className="w-6 h-6 text-green-600"/><h1 className="text-2xl font-bold">Tournament Viewer</h1><GlobalBadge/></div></div>
    {loading?<Card className="p-6 text-center text-zinc-500">Loading…</Card>:(
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <Card className="p-5">
          ...
          {completed.length?completed.map(f=>(<div key={f.id} className="py-2 border-b last:border-0"><div className="font-medium">{f.sides?.[0]} vs {f.sides?.[1]}</div><div className="text-sm text-zinc-500">{new Date(f.finishedAt||f.start).toLocaleString()}</div><div className="mt-1 text-sm"><span className="uppercase text-zinc-400 text-xs">Winner</span> <span className="font-semibold">{f.winner}</span> <span className="ml-3 font-mono">{f.scoreline}</span></div></div>)):<div className="text-zinc-500">No completed matches yet.</div>}
        </Card>
      </div>
    )}
  </div></div>);
...
/* -------- Fixtures (create/list/remove/update) -------- */
