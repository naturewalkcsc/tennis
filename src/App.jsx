...
const apiMatchesList = async () => {
  const r = await fetch('/api/matches?t=' + Date.now(), { cache: 'no-store' });
  if (!r.ok) return readLS(LS_MATCHES_FALLBACK, []);
  return r.json();
};
const apiMatchesAdd = async (payload) => {
  try {
    const r = await fetch('/api/matches?t=' + Date.now(), {
...
const GlobalBadge = () => {
  ...
  return (
    <span className={"ml-3 inline-flex items-center px-2 py-1 rounded-full text-xs " + (on ? "bg-emerald-100 text-emerald-700" : "bg-zinc-200 text-zinc-600")}
...
const Landing = ({ onStart, onResults, onSettings }) => {
  const Tile = ({ title, subtitle, icon: Icon, action, imgUrl }) => (
    <motion.button
      ...
      whileTap={{ scale: 0.99 }}
      className="w-full md:w-80 rounded-2xl overflow-hidden shadow-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 text-left flex flex-col h-full"
    >
      {/* Image (original colors, fixed height) */}
      <div className="w-full h-40 overflow-hidden">
        <img src={imgUrl} alt="" className="w-full h-full object-cover" />
      </div>

      {/* Text panel (inside the same button) */}
...
const Settings = ({ onBack }) => {
  ...
  const loadDraft = () => { try { const raw = localStorage.getItem(LS_PLAYERS_DRAFT); return raw ? JSON.parse(raw) : null; } catch { return null; } };
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (draft) {
        setSingles(draft.singles || []); setDoubles(draft.doubles || []);
        setDirty(true); setLoading(false); return;
      }
      try {
        const obj = await apiPlayersGet();
        if (!alive) return;
        setSingles(obj.singles || []); setDoubles(obj.doubles || []);
      } catch {
        const s = readLS(LS_SINGLES, ["Player A", "Player B", "Player C"]);
        const d = readLS(LS_DOUBLES, ["Team A/Team B"]);
        setSingles(s); setDoubles(d); setDirty(true);
        setError("Server unavailable. Using local draft — click Save when online.");
  ...
  const onChangeSingles = (idx, val) => setSingles(prev => { const ns = prev.map((v, i) => i === idx ? val : v); markDirty(ns, doubles); return ns; });
  const onChangeDoubles = (idx, val) => setDoubles(prev => { const nd = prev.map((v, i) => i === idx ? val : v); markDirty(singles, nd); return nd; });
  ...
  const doSave = async () => {
    ...
    try {
      ...
      saveDraft(singles, doubles); setDirty(true);
    } finally { setSaving(false); }
  };
  const handleBack = async () => { if (dirty && !saving) await doSave(); onBack(); };
  const refresh = async () => {
...
const MatchConfig = ({ onBack, onStartScoring }) => {
  ...
  useEffect(() => {
    let alive = true;
    (async () => {
      try { const obj = await apiPlayersGet(); if (alive) setPlayers(obj); }
      catch { const s = readLS(LS_SINGLES, []), d = readLS(LS_DOUBLES, []); if (alive) setPlayers({ singles: s, doubles: d }); }
    })();
    return () => { alive = false; };
  }, []);
  ...
  const [rule, setRule] = useState("regular");
  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      ...
      <Card className="p-6 space-y-6">
          ...
          <div className="space-y-4">
            <label className="flex items-start gap-3">
              ...
              <div>
                ...
                {rule === 'firstToGames' && (<div className="mt-3 w-40"><TextInput type="number" value={gamesTarget} onChange={setGamesTarget} min={1} max={6} placeholder="Enter 1–6" /></div>)}
              </div>
            </label>
            ...
            <label className="flex items-start gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-zinc-600 dark:text-zinc-400">Sets to 6 (win by 2), tie-break to 7 at 6–6, best of 3 default.</div>
              </div>
            </label>
            ...
            <label className="flex items-start gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-zinc-600 dark:text-zinc-400">Choose best-of-N sets (1–3).</div>
                {rule === 'bestOfSets' && (<div className="mt-3 w-40"><Select value={bestOf} onChange={setBestOf} options={['1', '3']} placeholder="Select 1 or 3" /></div>)}
              </div>
            </label>
          </div>
        </div>
        ...
        <div className="pt-2">
          <Button onClick={start} disabled={!canStart}><Play className="w-4 h-4" /> Start</Button>
        </div>
      </Card>
    </div>
  );
...
function advancePoint(a, b, who) {
  let pA = a, pB = b;
  if (who === 0) { if (pA === 40 && pB === 40) pA = "Ad"; else if (pA === "Ad") pA = "Game"; else if (pB === "Ad") pB = 40; else pA = nextPoint(pA); }
  else { if (pA === 40 && pB === 40) pB = "Ad"; else if (pB === "Ad") pB = "Game"; else if (pA === "Ad") pA = 40; else pB = nextPoint(pB); }
  return [pA, pB];
}
...
const Scoring = ({ config, onAbort, onComplete }) => {
  ...
  const tiebreakServer = () => {
    if (!currentSet.tie) return server;
    const tbPoints = currentSet.tieA + currentSet.tieB;
    const start = currentSet.tieStart ?? server;
    if (tbPoints === 0) return start;
    const block = Math.floor((tbPoints - 1) / 2) % 2;
  ...
  const pointTo = (logicalWho) => {
    ...
    if (currentSet.tie) {
      const ns = [...sets]; const setObj = { ...currentSet };
      if (who === 0) setObj.tieA++; else setObj.tieB++;
      if (setOver(setObj)) setObj.finished = true;
      ns[ns.length - 1] = setObj; setSets(ns);
      if (setObj.finished) { if (!gameTargetMode && (sets.length < effectiveBestOf)) setSets(prev => [...prev, makeEmptySet()]); startNextGameRotateServer(); }
      return;
    }
    ...
    const ns = [...sets]; const setObj = { ...currentSet };
    if (gw === "A") setObj.gamesA++; else setObj.gamesB++;
    setPoints([0, 0]);

    if (gameTargetMode) { if (setObj.gamesA === gamesTarget || setObj.gamesB === gamesTarget) { setObj.finished = true; } }
    else {
      if (setObj.gamesA === 6 && setObj.gamesB === 6) { setObj.tie = true; setObj.tieStart = server; }
      else if (setOver(setObj)) { setObj.finished = true; }
    }
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      ...
      <Card className="p-6">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="text-sm text-zinc-600 dark:text-zinc-300">
            Server: <span className="font-semibold">{tiebreakServer() === 0 ? sides[0] : sides[1]}</span>
            {currentSet.tie && <span className="ml-2 px-2 py-0.5 rounded bg-amber-100 text-amber-700 text-xs">Tie-break</span>}
          </div>
        </div>
      ...
      {matchDone && (<div className="mt-4 p-4 bg-emerald-600 text-white rounded-xl">Match complete! Saving result…</div>)}
    </div>
  );
};
...
const Results = ({ onBack }) => {
  ...
  const exportPDF = () => {
    if (!list.length) return;
    const doc = new jsPDF(); doc.setFontSize(14); doc.text("Tennis Match Results", 14, 16); doc.setFontSize(10);
