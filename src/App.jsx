...
import { Trophy, Play, Users, ChevronLeft, Plus, Trash2, CalendarPlus, RefreshCw, X } from "lucide-react";
...
/* -------- API wrappers (no Global badge/viewer now) -------- */
  ...
  const Tile=({title,subtitle,src,action})=>(
    <motion.button onClick={action} whileHover={{y:-2}} className="w-full md:w-80 rounded-2xl overflow-hidden border shadow bg-white text-left">
      <div className="h-40 relative">
        {/* Images are in public/ and must be referenced as absolute paths like /StartMatch.jpg */}
...
const Settings=({onBack})=>{
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      try{ const obj=await apiPlayersGet(); if(alive){ setSingles(obj.singles||[]); setDoubles(obj.doubles||[]) } }
...
function advancePoint(a,b,who){
  let pA=a,pB=b;
  if(who===0){
    if(pA===40&&pB===40){ pA="Ad" } else if(pA==="Ad"){ pA="Game" } else if(pB==="Ad"){ pB=40 } else { pA=nextPoint(pA) }
  }else{
    if(pA===40&&pB===40){ pB="Ad" } else if(pB==="Ad"){ pB="Game" } else if(pA==="Ad"){ pA=40 } else { pB=nextPoint(pB) }
  }
  return [pA,pB];
}
...
const Results=({onBack})=>{
  ...
  const completed=[...completedFixtures, ...matches.map(m=>({id:m.id,sides:m.sides,finishedAt:m.finishedAt,scoreline:m.scoreline,winner:m.winner,mode:m.mode||'singles'}))]
...
function AdminApp(){
  ...
  return (
    <div className="app-bg">
      ...
      <footer className="py-6 text-center text-xs text-zinc-500">© {new Date().getFullYear()} RNW NPL 2025</footer>
    </div>
  );
}

/* -------- Root (no /viewer route anymore) -------- */
