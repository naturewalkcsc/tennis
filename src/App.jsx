...
/* ------------------------------------------
   SIMPLE LOCAL ADMIN LOGIN (NO PREFILL)
-------------------------------------------*/
      ...
      <div className="max-w-sm mx-auto p-6">
        ...
        <div className="bg-white rounded-2xl border shadow p-5">
          <form onSubmit={submit} className="space-y-4">
            ...
            <button
              type="submit"
              className="w-full px-4 py-3 rounded-xl bg-green-600 text-white"
            >
              Enter Admin
            </button>
          </form>
...
/* ------------------------------------------------
   API WRAPPERS (players, fixtures, matches)
-------------------------------------------------*/
...
const Card = ({ className = "", children }) => (
  <div
    className={`bg-white rounded-2xl shadow border border-zinc-200 ${className}`}
  >
    {children}
  </div>
);
...
const Button = ({
  ...
  const base =
    "inline-flex items-center justify-center gap-2 px-5 py-3 rounded-xl font-medium";
  const styles = {
...
/* ------------------------------------------------
   LANDING (card images from src/assets)
-------------------------------------------------*/
  ...
  return (
    <div className="max-w-5xl mx-auto p-6">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        <Tile
          title="Start Match"
          subtitle="Choose from fixtures"
          src={imgStart}
          action={onStart}
        />
        <Tile
          title="Results"
          subtitle="Active • Upcoming • Completed"
          src={imgScore}
          action={onResults}
        />
        <Tile
...
const Settings = ({ onBack }) => {
  ...
  const saveDraft = (s, d) => {
    try {
      localStorage.setItem(
        LS_PLAYERS_DRAFT,
        JSON.stringify({ singles: s, doubles: d })
      );
    } catch {}
  };
  ...
  const delSingles = (i) => {
    const s = singles.filter((_, ix) => ix !== i);
    setSingles(s);
    markDirty(s, doubles);
  };

  const delDoubles = (i) => {
    const d = doubles.filter((_, ix) => ix !== i);
    setDoubles(d);
    markDirty(singles, d);
  };
  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      ...
      {error && (
        <Card className="p-4 mb-4 text-red-700 bg-red-50 border border-red-200 rounded-xl">
          {error}
        </Card>
      )}
              ...
              {singles.map((name, idx) => (
                <div key={idx} className="flex items-center gap-2">
                  <input
                    ...
                    value={name}
                    onChange={(e) => updSingles(idx, e.target.value)}
                  />
                  <button
                    onClick={() => delSingles(idx)}
              ...
              {doubles.map((name, idx) => (
                <div key={idx} className="flex items-center gap-2">
                  <input
                    className="flex-1 rounded-xl border px-3 py-2"
                    value={name}
                    onChange={(e) => updDoubles(idx, e.target.value)}
                  />
                  <button
                    onClick={() => delDoubles(idx)}
                    className="px-3 py-2 rounded-xl hover:bg-zinc-100"
                  >
                    <Trash2 className="w-4 h-4" />
                  </button>
...
/* ------------------------------------------------
   START MATCH SCREEN - FROM FIXTURES ONLY
-------------------------------------------------*/
function StartFromFixtures({ onBack, onStartScoring }) {
  ...
  const startFixture = async (fx) => {
    ...
    // If admin selects a future fixture, reset start time to now
    if (fx.start > now) patch.start = now;

    // Only one active match allowed
...
function Scoring({ config, onAbort, onComplete }) {
  const { sides, rule, bestOf, gamesTarget, startingServer, fixtureId } =
    config;

  const effectiveBestOf =
    rule === "bestOfSets" ? bestOf : rule === "regular" ? 3 : 1;
  ...
  const matchDone = (() => {
    if (gameTargetMode)
      return (
        currentSet.finished &&
        (currentSet.gamesA === gamesTarget || currentSet.gamesB === gamesTarget)
      );
  ...
  const pointTo = (who) => {
    ...
    if (gameTargetMode) {
      if (so.gamesA === gamesTarget || so.gamesB === gamesTarget)
        so.finished = true;
    } else {
      if (so.gamesA === 6 && so.gamesB === 6) {
        so.tie = true;
        so.tieStart = server;
      } else if (setOver(so)) {
        so.finished = true;
    ...
    if (
      so.finished &&
      !gameTargetMode &&
      winnerSets(ns).A < targetSets &&
      winnerSets(ns).B < targetSets
    ) {
  ...
  const recordResult = async () => {
    const sl = sets
      ...
      .map((s) =>
        s.tie
          ? `${s.gamesA}-${s.gamesB}(${Math.max(s.tieA, s.tieB)})`
          : `${s.gamesA}-${s.gamesB}`
      )
      .join(" ");
    ...
    const winner =
      setsA > setsB
        ...
        : setsB > setsA
        ? sides[1]
        : currentSet.gamesA > currentSet.gamesB
        ? sides[0]
        : sides[1];
          ...
          <div className="text-sm font-mono">
            {sets.map((s, i) => (
              <span key={i} className="inline-block mr-3">
                {s.tie
                  ? `${s.gamesA}-${s.gamesB} TB ${s.tieA}-${s.tieB}`
                  : `${s.gamesA}-${s.gamesB}`}
              </span>
            ))}
          </div>
          ...
          <Card className="p-5">
            ...
            {active.length ? (
              active.map((f) => (
                <div
                  key={f.id}
                  className="py-2 border-b last:border-0 flex items-center gap-2"
                >
