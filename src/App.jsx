...
/* ---- API helpers (Vercel serverless endpoints) ---- */
...
const apiMatchesList = async () => {
  try { const r = await fetch("/api/matches" + buster(), { cache: "no-store" }); if (!r.ok) throw 0; return await r.json(); }
...
const apiFixturesAdd = async (payload) => {
  ...
  if (!r.ok) throw new Error("fixtures add");
};
const apiFixturesRemove = async (id) => {
  ...
  if (!r.ok) throw new Error("fixtures remove");
};
const apiFixturesClear = async () => {
  ...
  if (!r.ok) throw new Error("fixtures clear");
};
...
const Select = ({ value, onChange, options, placeholder }) => (
  <select value={value} onChange={(e) => onChange(e.target.value)}
          className="w-full mt-1 rounded-xl border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 px-3 py-2">
...
const GlobalBadge = () => {
  ...
  useEffect(() => {
    let alive = true;
    (async () => { const ok = await apiStatus(); if (alive) setOn(ok); })();
    return () => { alive = false };
  }, []);
  return (
    <span className={"ml-3 inline-flex items-center px-2 py-1 rounded-full text-xs " +
      (on ? "bg-emerald-100 text-emerald-700" : "bg-zinc-200 text-zinc-600")}
      title={on ? "Global sync: ON" : "Global sync: OFF"}>
...
const ThemeToggle = () => {
  ...
  useEffect(() => { document.documentElement.classList.toggle("dark", dark); writeLS(LS_THEME, dark); }, [dark]);
  return <Button variant="ghost" onClick={() => setDark(d => !d)} className="!px-3 !py-2" title="Toggle theme">{dark ? "🌙" : "☀️"}</Button>;
};

/* ---- Landing (with the buttons/tiles) ---- */
...
const Settings = ({ onBack }) => {
  ...
  const doSave = async () => {
    ...
    try {
      ...
      setToast({ show: true, text: "Players saved" }); setTimeout(() => setToast({ show: false, text: "" }), 1200);
    } catch {
      setError("Save failed. Keep editing and try again."); saveDraft(singles, doubles); setDirty(true);
...
const Fixtures = ({ onBack }) => {
  ...
  useEffect(() => {
    let alive = true;
    (async () => {
      try { const p = await apiPlayersGet(); if (!alive) return; setPlayers(p) } catch { }
      try { const fx = await apiFixturesList(); if (!alive) return; setList(fx) }
      catch { setError("Could not load fixtures (KV off?).") }
      finally { if (alive) setLoading(false) }
    })();
    return () => { alive = false }
  }, []);
  ...
  const addFixture = async (e) => {
    ...
    } catch { setError("Add failed. Check KV variables."); }
  };
  const removeFx = async (id) => { try { await apiFixturesRemove(id); setList(prev => prev.filter(f => f.id !== id)) } catch { } };
  const clearAll = async () => { if (!confirm("Clear ALL fixtures?")) return; try { await apiFixturesClear(); setList([]) } catch { } };
  const refresh = async () => { try { const fx = await apiFixturesList(); setList(fx) } catch { } };
      ...
      <Card className="p-5 mb-6">
        ...
        <form onSubmit={addFixture} className="grid md:grid-cols-4 gap-4">
          ...
          <div className="md:col-span-1 grid grid-cols-2 gap-2">
            <div>
              <div className="text-sm text-zinc-600 dark:text-zinc-400 mb-1">Date</div>
              <TextInput type="date" value={date} onChange={setDate} />
            </div>
            <div>
              <div className="text-sm text-zinc-600 dark:text-zinc-400 mb-1">Time</div>
        ...
        <div className="space-y-3">
          {list.map(f => (
            <Card key={f.id} className="p-4 flex items-center gap-4">
              ...
              <Button variant="ghost" onClick={() => removeFx(f.id)} title="Remove"><X className="w-4 h-4" /></Button>
            </Card>
          ))}
        </div>
      )}
...
function setOver(set) {
  if (set.tie) { if ((set.tieA >= 7 || set.tieB >= 7) && Math.abs(set.tieA - set.tieB) >= 2) return true; return false }
...
const Scoring = ({ config, onAbort, onComplete }) => {
  ...
  const tiebreakServer = () => {
    ...
    const tb = currentSet.tieA + currentSet.tieB;
    const start = currentSet.tieStart ?? server;
    if (tb === 0) return start;
    const block = Math.floor((tb - 1) / 2) % 2;
    ...
    <div className="max-w-4xl mx-auto p-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="ml-auto text-sm text-zinc-600 dark:text-zinc-300">
          <span>
            {rule === "firstToGames"
              ? `First to ${gamesTarget} games`
              : rule === "bestOfSets"
                ? `Best of ${effectiveBestOf} sets`
                : "Regular (Best of 3)"}
          </span>
        </div>
      </div>
      ...
      <Card className="p-6">
        ...
        <div className="grid grid-cols-5 gap-4 items-center mt-4">
          <div className="col-span-2 text-right pr-4">
            <div className="text-lg font-semibold truncate flex items-center justify-end gap-2">
              {leftName} {leftIsServer && <span title="Serving now" className="inline-block w-2 h-2 rounded-full bg-emerald-500" />}
            </div>
          </div>
          ...
          <div className="col-span-2 pl-4">
            <div className="text-lg font-semibold truncate flex items-center gap-2">
              {rightName} {rightIsServer && <span title="Serving now" className="inline-block w-2 h-2 rounded-full bg-emerald-500" />}
            </div>
          </div>

          <div className="col-span-2 text-right pr-4 text-3xl font-bold">{String(flipped ? points[1] : points[0])}</div>
          <div className="col-span-1 text-center">—</div>
          <div className="col-span-2 pl-4 text-3xl font-bold">{String(flipped ? points[0] : points[1])}</div>
        </div>
...
const Results = ({ onBack }) => {
  ...
  const exportPDF = () => {
    if (!list.length) return;
    const doc = new jsPDF();
    doc.setFontSize(14); doc.text("Tennis Match Results", 14, 16); doc.setFontSize(10);
    let y = 24; const lh = 6;
    const addLine = t => { doc.text(t, 14, y); y += lh; if (y > 280) { doc.addPage(); y = 20 } };
    list.forEach((m, i) => {
...
function MatchConfig({ onBack, onStartScoring }) {
  ...
  useEffect(() => {
    let alive = true;
    (async () => { try { const p = await apiPlayersGet(); if (alive) setPlayers(p) } catch { } })();
    return () => { alive = false }
  }, []);
  ...
  const start = () => onStartScoring({
    mode, sides: [p1, p2], rule,
    gamesTarget: Number(gamesTarget), bestOf: Number(bestOf),
    createdAt: Date.now(), startingServer: 0
  });
  return (
    <div className="max-w-3xl mx-auto p-6">
      <div className="flex items-center gap-3 mb-6">
      ...
      <Card className="p-6 space-y-6">
        ...
        <div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              <div className="text-sm text-zinc-600 dark:text-zinc-400 mb-1">{mode === "singles" ? "Player 1" : "Team 1"}</div>
              <Select value={p1} onChange={setP1} options={opts} placeholder="Choose..." />
            </div>
            <div>
              <div className="text-sm text-zinc-600 dark:text-zinc-400 mb-1">{mode === "singles" ? "Player 2" : "Team 2"}</div>
              <Select value={p2} onChange={setP2} options={opts} placeholder="Choose..." />
            </div>
          </div>
        </div>
        <div>
  ...
  return (
    <div className="app-bg">
      ...
      <footer className="py-6 text-center text-xs text-zinc-500">
        © {new Date().getFullYear()} Lawn Tennis Scoring
      </footer>
    </div>
  );
}
