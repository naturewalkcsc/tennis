...
export default async function handler(req,res){
  ...
  try{
    ...
    if(req.method==='POST'){
      ...
      if(action==='add'){ const f=body.payload||{}; const list=(await redis.get(key))||[]; list.push({status:f.status||'upcoming',...f}); await redis.set(key,list); return res.status(200).json({ok:true}) }
